...
def get_response_dict_informal():
    ...
    bot_texts[0][8] = Reply(bot_id=0, response_id=8, texts={modes.GENERAL:[["Cool, looks like you have a plan B", "Just remember, even though you cannot control everything, there is a way to get back on your feet", "Sounds good?"]]}, next_id=config.QUESTION_INDEX)
    ...
    tmp_text = {}
    tmp_text[modes.GENERAL] = [["It seems like I might not be the right bot for you", "If you\'d like to talk to one of my friends say '/switch'", "Otherwise, just type goodbye to end the conversation"]]
    # tmp_text[topics.TRAFFIC] = [["Maybe taking a different route or leaving earlier or later could solve your problem?", "Spending some time planning your travel may help you come up with a solution"]]
    # tmp_text[topics.TIRED] = [["The easiest solution would be to just go to sleep, but if this doesn\'t work for you spend some time thinking of ways to re-organize your schedule to accommodate a good night\'s rest"]]
    # tmp_text[topics.LATE] = [["Everyone\'s late once in awhile", "However, if it is a common occurence maybe think about leaving earlier or taking a quicker route to make your scheduled events on time"]]
    # tmp_text[topics.DRIVER] = [["If aggressive driving is a frequent issue, then perhaps taking a different route could solve your problem"]]
    ...
    #good food = 3
    #{1, 1, 1}
    bot_texts[8][9] = Reply(bot_id=8, response_id=9, texts={modes.GENERAL:[["I\'m glad that you have been able to talk with friends and/or family about this","Wonderful, it seems as though this problem isn\'t affecting these three major aspects of your life. Great job staying on top of your sleeping and eating as well as reaching out to friends and family for support" , "Sounds good?"]]}, next_id=config.QUESTION_INDEX)
