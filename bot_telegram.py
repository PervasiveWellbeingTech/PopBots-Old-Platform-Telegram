...
Notes: This version may have (very) not-up-to-date documentation.
...
#     def text_callback(self, bot, update):
#         """
...
def text_test(bot, update):
    bot.send_message(chatid=update.message.chat_id, text='Hi!')



if __name__ == '__main__':
    # Telegram Bot Authorization Token
    #bot = TelegramBot() #bot for testing only
    #bot = TelegramBot('676639758:AAFrOKaCJAzBOO-7LM2W3p4Ie1Rkf9O6qsU')
    #run(bot, '660721089:AAFFtzkiZVC96U_Cqzt3Y3sW_BsHaFyJfFY')
    updater = Updater('660721089:AAFFtzkiZVC96U_Cqzt3Y3sW_BsHaFyJfFY')
    dp = updater.dispatcher # Get the dispatcher to register handlers
    dp.add_handler(MessageHandler('start',text_test))
    dp.add_handler(MessageHandler(Filters.text, text_test))
